...
export default function BusinessMain() {
  ...
  const { data: myBusinesses, isPending: myBusinessesisPending } = useQuery({
    queryKey: ["myBusinesses"],
    queryFn: () =>
      axiosBase.get(`/businesses/${auth.user._id}`, {
        headers: { Authorization: auth.access_token },
      }),
    select: (data) =>
      data.data?.business.filter((item) =>
  ...
  return (
    <section className="[grid-area:main]">
      <Tabs
        defaultValue="all"
        onValueChange={() => setIsMyBusinesses(!isMyBusinesses)}
      >
